...
public class MainFrame extends JFrame {
	...
	private int currentPageNumber=1;//初始化表格中的当前页数为1
...
//	}
	...
	JButton button=new JButton("");//"查看历届开奖按钮"
	//设置"查看历届开奖按钮"的图标
	button.setIcon(
			new ImageIcon(MainFrame.class.getResource("/img_btn/09.png"))
			);
	button.setBounds(6,74,184,40);//"查看历刷子开奖"按钮的位置及宽高
	contentPane.add(button);//将"查看历届开奖"按钮添加到处定义背景面板中
	...
	JButton button_5=new JButton("");//中奖查询扫钮
	...
	contentPane.add(scrollPane);//将流动面板添加到自定义背景面板中
	table=new JTable();//表格模型
	scrollPane.setViewportView(table);//向滚动面板中添加表格
	
	firstPageButton=new JButton("首页");//"首页"按钮
	//设置"首页"按钮的图标
	...
	nextPageButton.setBounds(686,439,84,27);//"下一页按钮的位置及宽高"
	contentPane.add(nextPageButton);//将"下一页"按钮添加到自定义背景面板中
	
	lastPageButton=new JButton("尾页");//"尾页"按钮
	//设置"尾页"按钮的图标
	...
	firstPageButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e){
			do_firstPageButton_actionPerformed(e);//为"首页"添加动作事件的监听
	...
	protected void do_lastPageButton_actionPerformed(ActionEvent e){
		...
		table.setModel(newModel);//设置表格模型
		firstPageButton.setEnabled(true);//启用"首页"按钮
		latePageButton.setEnabled(true);//户用"上一页"按钮
		nextPageButton.setEnabled(false);//禁用"下一页"按钮
		lastPageButton.setEnabled(false);//禁用"尾页"按钮
	...
	public void selecttable(){//分页显示开奖号码的方法
		...
		if(maxrows !=0){//判断是否有数据执行下面的方法
			...
			try{
				//为表格中每一行的单元格赋值
				while (rs.next()){
					defaultModel.addRow(new Object[]{rs.getInt(2),
