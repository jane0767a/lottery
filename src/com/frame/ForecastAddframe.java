...
public class ForecastAddframe extends JDialog implements Runnable, ActionListener {// 随机选号对话框
	...
	public void run() {
		...
		if(rdbtnHave.isSelected() && tf_1.getText().equals("") && tf_2.getText().equals("")
			...
			but.setEnabled(true);//设置"随机一注"按钮可用
			return;
		}
		
		//单选按钮"有"被选中且文本框中的数组之和大于7
		if(rdbtnHave.isSelected() && (exchangeInteger(tf_1) + exchangeInteger(tf_2)
		+exchangeInteger(tf_3)
		+exchangeInteger(tf_4)+exchangeInteger(tf_5)+exchangeInteger(tf_6)
		+exchangeInteger(tf_7)+exchangeInteger(tf_8)+exchangeInteger(tf_9)
				+exchangeInteger(tf_0))>7){
			JOptionPane.showMessageDialog(null, 
					"警告:号码个数多于7个!",
					"警告",
					JOptionPane.WARNING_MESSAGE);//弹出提示框
			...
			while (bol) {
				try {
					...
					case 1000:// 1000毫秒时
						...
						break;
					case 1500:// 1500毫秒时
						s = sevenTextField.getText();// 获取"选号号码"文本框中的第2位奖号
						sevenTextField.setText(s + c);// 把第3位的值添加到"选号号码"文本框中
						break;
					case 2000:// 2000毫秒时
						s = sevenTextField.getText();// 获取"选号号码"文本框中的第3位奖号
						sevenTextField.setText(s + d);// 把第4位的值添加到"选号号码"文本框中
						break;
					case 3000:// 3000毫秒时
						s = sevenTextField.getText();// 获取"选号号码"文本框中的第4位奖号
						sevenTextField.setText(s + e);// 把第5位的值添加到"选号号码"文本框中
						break;
					case 4000:// 4000毫秒时
						s = sevenTextField.getText();// 获取"选号号码"文本框中的第5位奖号
						sevenTextField.setText(s + f);// 把第6位的值添加到"选号号码"文本框中
						break;
					case 5000:// 5000毫秒时
						s = sevenTextField.getText();// 获取"选号号码"文本框中的第6位奖号
		...
		if (rdbtnHave.isSelected()) {// 单选按钮"有被选中"
			...
			for (int i = 0; i < str.length; i++) {
				String[] text = str[i].split(": ");// 拆分析数组中的元素
				if (text.length == 2) {
					int number = Integer.parseInt(text[0]);// 将文本框中的数字个数转换为int型
			...
			while (list.size() < 7) {
				...
				if (list.contains(num)) {// 集合中包含随机生成的整数
			...
			List<Integer> indexs = new ArrayList<>();// "号码"集合元素索引的集合
			// 集合中的元素个数小于7
			while (indexs.size() < 7) {
				int index = ram.nextInt(7);// 随机生成一个在0~6范围内的整数
				if (indexs.contains(index)) {// 集合中包含随机生成的整数
					continue;// 跳过本次循环，执行下一次循环

				} else {// 集合中不包含随机生成的整数
					indexs.add(index);// 向集合indexs中添加元素
	...
	public void actionPerformed(ActionEvent e) {
		...
		bol = true;
		index = 0;// 为奖号变换时间赋值
		but.setEnabled(false);// 设置"随机一注"按钮不可用
	...
	protected void do_btnNewButton_actionPerformed(ActionEvent e) {
		...
		if(validate.validateNumber(sevenTextField.getText())){
			...
			Boolean b;
			b=fr.addForecastDao(Integer.parseInt(numberTextField.getText()),sevenTextField.getText());
			//获得添加购买号码的返回值
			if(b){//添加购买号码成功
	...
	private String itemEvent(JCheckBox ckBox, JTextField tf) {
		...
		return ckBox.getText() + ": " + tf.getText();// 返回复选框文本框中的文本内容
