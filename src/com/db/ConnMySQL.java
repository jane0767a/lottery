...
public class ConnMySQL {
	...
	public static boolean creatConnection(){//建立MySQL数据库的连接
		...
		try {
			//根据连接MySQL数据库的路径，用户名和密码连接MySQL数据库
	...
	public int getABC(String abc, int number){//获取进度条，统计标签中的数据
		//获得number(0-9)在历届开奖在号码中第abc(a-g)位出现的次数
		...
		public static List<History> getFirstTenData(){//获得最近10期的开奖结果
			...
			List<History> list=new ArrayList<>();//存储最近10期的开奖结果的集合
			Statement statement=null;//声明用于执行SQL语句的接口
			if(con==null){//Connection对象为空
				creatConnection();//建立MySQL数据库的连接
			...
			try{
				...
				ResultSet rs=statement.executeQuery(sql);//执行查询语句获得结果集
				while(rs.next()){//遍历结果集
					History history=new History();//创建历届开奖在结果对象
					history.setNumber(rs.getInt(2));//获得开次期数
		...
		public Boolean addForecast(Forecast fr){//添加机选号码
			...
			try{
				...
				statement.setInt(1, fr.getNumber());//设置预处理训句的参数值
